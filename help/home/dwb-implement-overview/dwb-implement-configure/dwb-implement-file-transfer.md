---
description: Eine schnelle Anleitung für verschiedene Dateiübertragungsmethoden in DWB.
...
Eine schnelle Anleitung für verschiedene Dateiübertragungsmethoden in DWB.

File Transfer Governance ist ein Standardprozess, um Dateien von einem internen Ordner auf einen anderen Server oder interne Dateiverschiebungen zu übertragen.

## Verschiedene Dateiübertragungsmethoden {#section-972bb39ba1954c6ebd35f6d042593efe}

1. AWS (Amazon Web Services)

   1. Heben Sie ein Ticket auf, um die AWS-Befehlszeilenschnittstelle auf dem Server zu installieren, falls noch nicht installiert (siehe [http://docs.aws.amazon.com/cli/latest/userguide/installing.html](http://docs.aws.amazon.com/cli/latest/userguide/installing.html)).
   1. Wie zu überprüfen? Versuchen Sie, AWS mithilfe der Eingabeaufforderung zu konfigurieren (siehe [http://docs.aws.amazon.com/cli/latest/userguide/cli-chap-getting-started.html](http://docs.aws.amazon.com/cli/latest/userguide/cli-chap-getting-started.html)).

1. Übertragen Sie Dateien vom FTP-Server in das NAS-Verzeichnis.

   1. FTP-Offline-Feeds vom FTP-Server zum NAS-Ordner. Die folgenden Details sind für FTP erforderlich.
      ...
      (FTP-Details stehen in der Projektprüfliste zur Verfügung. Externen FTP-Benutzer zum Übertragen der Dateien verwenden)

   1. Verwenden Sie das unten angehängte Skript ftp_winscp_get.pl und planen Sie es je nach Anforderung.

      ftp_winscp_get.pl

      Dieses Skript sollte unter E:\scripts\Scripository\Library\Perl abgelegt werden.

      >[!NOTE]
      >
      >Wenn der Ordner &quot;Scrirepository&quot;nicht verfügbar ist, finden Sie weitere Informationen unter [Wöchentliche Neuverarbeitung](../../../home/dwb-implement-overview/dwb-implement-configure/dwb-implement-reprocess-scripting.md#concept-60529e12d6d94386a02c1c6fdedf0295) zum Herunterladen des Ordners.

   1. Planen Sie das Skript basierend auf der Dateiverfügbarkeit unter ftp_address.
   1. Die Benennungskonvention der Datei sollte &quot;JJJMMTT-&lt;offline_feed_name>-00&quot;lauten.*

1. Übertragen Sie Dateien vom NAS-Verzeichnis auf den FTP-Server.

   1. Verwenden Sie ftp_winscp_put.pl-Skript und Zeitplan basierend auf Anforderungen.
...
1. Übertragen Sie Dateien von einem NAS-Verzeichnis in einen anderen NAS-Ordner.

   1. Kopieren Sie die Datei und fügen Sie sie direkt eine Verbindung zu einem NAS-Verzeichnis von einem anderen. Gehen Sie wie folgt vor:)

      Anmelden beim Server -> Gehen Sie zum Ausführen -> \\server_name\E$ [neuer Ordner wird geöffnet und kopiert oder verschoben die Dateien]

   1. Verwenden Sie das Skript &quot;copy_files.pl&quot;, um Dateien von einem Server auf einen anderen zu kopieren, oder &quot;move_files.pl&quot;, um Dateien von einem Server auf einen anderen zu verschieben. (Diese Dateien stehen unter E:\scripts\Scripository zur Verfügung.)
