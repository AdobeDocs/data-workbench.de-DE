...
Informationen zur Ansicht früherer Funktionen und Fehlerbehebungen in früheren Versionen finden Sie im Archiv [Versionshinweise](https://docs.adobe.com/content/help/de-DE/data-workbench/using/release-notes/release-notes.html).
...
|--- |--- |
| [Exportieren in Analytics Core Services](/help/home/c-release-notes-insight/c-6-4/dwb-crs-integration.md) | Mit der Funktion zum Exportieren von Kundendatensätzen (Customer Record Service, CRS) können Sie Data Workbenchs aus der Detailtabelle in die Adobe Analytics Core Services exportieren, um sie in andere Analytics-Funktionen, einschließlich Reports &amp; Analysen, zu integrieren. |
...
| [Sperren von Profilen in der Workstation](/help/home/c-get-started/c-admin-intrf/c-prof-mgr/dwb-profile-lock.md) | Sie können verhindern, dass Profil von Benutzern überschrieben werden, die im Profil-, Dimensionen-, Berichte-, Arbeitsflächen-, Metriken- oder Filter-Manager arbeiten. Speichern Sie im Profil-Manager die Datei &quot;Internal.cfg&quot;in einem benutzerdefinierten Profil, um zu verhindern, dass mehrere Systemdateien auf dem Profil überschrieben werden. |
| [Neue Funktionen der Benutzeroberfläche](/help/home/c-adobe-data-workbench-dashboard/t-getting-started/c-dashboard-user-interface.md) | Data Workbench 6.4 fügt der Hilfe neue Workspace-Symbole, QuickInfos, Begrüßungsbildschirme und einen F1-Tastaturbefehl hinzu. Sie können auch Ihre Protokolldateien öffnen, indem Sie auf der Symbolleiste Hilfe > Ablaufverfolgungsverzeichnis öffnen auswählen. |
...
>[!NOTE]
>
>Sie können die Symbole in der Symbolleiste deaktivieren und zu den Textnamen zurückkehren. hinzufügen Sie den **[!DNL Toolbar Icons = bool: false]** Parameter in die Datei [](https://docs.adobe.com/content/help/en/data-workbench/using/client/c-insght-config-param.html)Insight.cfg. False deaktiviert die Symbole in der Benutzeroberfläche der Workstation und zeigt die Namen der Menüs in der Symbolleiste in Text an.
...
Ab Version 6.4 der Data Workbench können Sie die Standardgrenze für zwischengespeicherte Elemente von &quot;100000&quot;außer Kraft setzen, indem Sie deren Wert ändern und die Datei speichern. Der Standardwert wurde ausgewählt, um den meisten Anforderungen zu entsprechen.
...
Befolgen Sie beim Upgrade auf Data Workbench 6.4 diese Anforderungen und Empfehlungen.
...
* **hinzufügen** ***Ausgeschlossene Prozesse*** für den *MS System Center-Endpunktschutz in Windows 2012-Servern* für die folgenden ausführbaren Dateien:
   ...
   * **`ExportIntegration.exe`**
   ...
   Dadurch werden Weiße Liste-Rechte für diese miteinander verbundenen ausführbaren Dateien ermöglicht.

* **Aktualisieren Sie das *`Trust_ca_cert.pem`*Zertifikat auf den Servern**.
* **Reorganisation der Attribution-Profile**.

   * Der *Ordner &quot;Attribution* &quot;wurde in &quot; ***Attribution - Premium*** &quot;umbenannt (in der Standardinstallation unter *Profilen* \ *Attribution - Premium* gefunden).

   * Das *Premium* -Profil wurde entfernt und der Arbeitsbereich wurde in den neuen ***Ordner Attribution - Premium*** verschoben.

* **Aktualisieren Sie *die Einstellungen*Attribution-Premium**. Wenn Sie benutzerdefinierte Profil mit Parametereinstellungen haben, die das standardmäßige *Adobe SC* -Profil außer Kraft setzen, müssen Sie die benutzerdefinierten Felder in den Konfigurationsdateien aktualisieren:
...
* **hinzufügen Schriftarten auf dem Berichtsserver.** Fügen Sie in der Datei **`ReportServer.cfg`**die folgenden Schriftarten hinzu (für alle Sprachen):
...
* **Aktualisieren Sie Ihre Version von Microsoft Excel** (falls erforderlich).
   ...
   Mit der Veröffentlichung von Data Workbench 6.4 wurde die Excel 2007-Unterstützung eingestellt. Da die Data Workbench nur unter Microsoft Windows für 64-Bit-Architektur ausgeführt wird, sollten Sie auch eine 64-Bit-Version von Microsoft Excel installieren.
...
* Die *Übergeordnet Marketing Profil Export* -Funktion in der Detailtabelle wurde in Export von ***Profilen und Audiencen*** umbenannt.
* Die *Test- und Zielgruppen-Exportfunktion* in der Detailtabelle wurde in ***Adobe Target Export*** umbenannt.
...
* Der Export von Segmenten der Zielgruppe schlug fehl, da das Feld mboxPC fehlte. Dieser Fehler wurde nun behoben.
...
* Bei der Eingabe von nicht autorisierten Zeichen ( *oder*) als Spaltennamen in die`[CR]` Profil- und Audiencen-Exportdatei `[TAB]`werden falsche Protokolle erzeugt, sodass Daten nicht korrekt exportiert werden.
