...
Die [!DNL Categorize] -Transformation verwendet eine zweispaltige Nachschlagetabelle, die aus Musterzeichenfolgen-/Wertpaaren besteht. Während dieser Transformation liest der Data Workbench-Server jeden Ereignisdatensatz abwechselnd und vergleicht den Inhalt eines angegebenen Felds im Datensatz mit jeder der in der ersten Spalte der Suchtabelle aufgelisteten Musterzeichenfolgen. Wenn das angegebene Feld mit einer der Musterzeichenfolgen übereinstimmt, schreibt der Data Workbench-Server den Wert (in der zweiten Spalte zu finden), der mit dieser Musterzeichenfolge verknüpft ist, in ein bestimmtes Ausgabefeld im Datensatz.
...
A [!DNL Categorize] Die Umwandlung ist im Allgemeinen einfacher und schneller als die Verwendung einer [!DNL Regular Expression] Transformation, um dasselbe zu erreichen.

>[!NOTE]
>
>Der in [!DNL Categorize] die Groß-/Kleinschreibung beachten, es sei denn, es wird anders unter Verwendung der Variablen [!DNL Case Sensitive] Parameter.
...
<table id="table_1773344FAAE34BD4919CC4414249FDEE"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> True oder false. Gibt an, ob beim Teilzeichenfolgentest die Groß-/Kleinschreibung beachtet wird. </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Zeichenfolge, die verwendet wird, um die Spalten in der Lookup-Datei zu trennen. Muss ein einzelnes Zeichen in der Länge sein. </p> <p> Wenn Sie die Strg-Taste gedrückt halten und mit der rechten Maustaste im Trennzeichen-Parameter klicken, wird ein <span class="wintitle"> Einfügen</span> angezeigt. Dieses Menü enthält eine Liste von Sonderzeichen, die häufig als Trennzeichen verwendet werden. </p> </td> 
   <td colname="col3"> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Mehrere Werte </td> 
   <td colname="col2"> True oder false. Wenn der Wert "true"lautet und mehrere Zeilen in der Datei mit der Eingabe übereinstimmen, führt jede Übereinstimmung dazu, dass ein Wert an den Ausgabevektor von Zeichenfolgen angehängt wird. Wenn "false", wird nur die erste übereinstimmende Zeile in der Datei in der Ausgabe verwendet. Wenn die Eingabe ein Vektor ist, ist die Ausgabe in letzterem Fall auch ein Vektor der äquivalenten Länge. Wenn es sich bei der Eingabe um eine einfache Zeichenfolge handelt, ist die Ausgabe auch eine einfache Zeichenfolge. </td> 
...
* Änderungen an Lookup-Dateien in [!DNL Categorize] in der Variablen [!DNL Transformation.cfg] oder in einer [!DNL Transformation Dataset Include] -Datei muss der Datensatz umformatiert werden. Suchen nach Dateien für [!DNL Categorize] in der Variablen [!DNL Log Processing.cfg] oder [!DNL Log Processing Dataset Include] -Datei unterliegen dieser Einschränkung nicht. Informationen zur erneuten Verarbeitung Ihrer Daten finden Sie unter [Wiederaufbereitung und erneute Umwandlung](../../../../home/c-dataset-const-proc/c-reproc-retrans/c-unst-reproc-retrans.md).

* [!DNL Categorize] in der Variablen [!DNL Log Processing.cfg] oder [!DNL Log Processing Dataset Include] -Datei ihre Lookup-Dateien immer dann neu laden, wenn sich die Lookup-Dateien ändern. Änderungen werden nicht rückwirkend angewendet, sondern gelten für alle Protokolldaten, die nach der Änderung gelesen werden.

Dieses Beispiel veranschaulicht die Verwendung des [!DNL Categorize] Umwandlung zur Integration von Lookup-Daten mit Ereignisdaten, die aus dem Website-Traffic erfasst wurden. Angenommen, eine bestimmte Website enthält Geschäftsbereiche, und es ist erforderlich, dass sie sich ansehen und Vergleiche auf der Grundlage von Traffic-Fluss und -Wert vornehmen kann, der von den verschiedenen Abschnitten generiert wurde. Sie können eine Lookup-Datei erstellen, in der die Unterzeichenfolgen aufgelistet werden, die zur Identifizierung dieser verschiedenen Abschnitte verwendet werden.
...
* Änderungen an Lookup-Dateien in [!DNL FlatFileLookup] in der Variablen [!DNL Transformation.cfg] Datei oder [!DNL Transformation Dataset Include] -Dateien müssen neu umgewandelt werden. Suchen nach Dateien für [!DNL FlatFileLookup] in der Variablen [!DNL Log Processing.cfg] Datei oder [!DNL Log Processing Dataset Include] -Dateien unterliegen nicht dieser Beschränkung. Informationen zur erneuten Verarbeitung Ihrer Daten finden Sie unter [Wiederaufbereitung und erneute Umwandlung](../../../../home/c-dataset-const-proc/c-reproc-retrans/c-unst-reproc-retrans.md).

* [!DNL FlatFileLookup] Transformationen in der [!DNL Log Processing.cfg] Datei oder [!DNL Log Processing Dataset Include] -Dateien laden ihre Lookup-Dateien jedes Mal neu, wenn sich die Lookup-Dateien ändern. Änderungen werden nicht rückwirkend angewendet, sondern gelten für alle Protokolldaten, die nach der Änderung gelesen werden.

Dieses Beispiel veranschaulicht die Verwendung des [!DNL FlatFileLookup] Umwandlung zur Integration von Lookup-Daten mit Ereignisdaten, die aus dem Website-Traffic erfasst wurden. Angenommen, Sie möchten Website-Partner isolieren, die den Traffic an die Website weiterleiten, und ihre Partner-IDs in benutzerfreundlichere Namen umwandeln. Anschließend können Sie die benutzerfreundlichen Namen verwenden, um erweiterte Dimensionen und Visualisierungen zu erstellen, die der Geschäftsbeziehung besser zugeordnet sind als die Site-zu-Site-Beziehung, die für das Routing von Traffic verwendet wird.

Die Beispielumwandlung durchsucht das Feld cs(referrer-query) nach dem Name-Wert-Paar PartnerID und, falls vorhanden, nach der Lookup-Datei [!DNL Lookups\partners.txt] wird verwendet, um den PartnerID-Wert mit den Werten im [!DNL Partner] -Spalte der Tabelle. Wenn sich eine Zeile befindet, erhält das Ausgabefeld x-partner-name den Namen aus der [!DNL PrintName] Spalte der identifizierten Zeile.
...
Die [!DNL ODBCLookup] Umwandlung funktioniert wie [!DNL FlatFileLookup] Umwandlung. Der einzige Unterschied besteht darin, dass die bei dieser Umwandlung verwendete Suchtabelle aus einer ODBC-Datenbank und nicht aus einer reduzierten Datei gefüllt wird.

>[!NOTE]
>
>[!DNL ODBCLookup] Transformationen können nur während der Umwandlungsphase des Datensatzerstellungsprozesses ausgeführt werden. Wenn möglich, empfiehlt Adobe, die [!DNL FlatFileLookup] Umwandlung anstelle der [!DNL ODBCLookup] Umwandlung. [!DNL FlatFileLookup] Transformationen sind von Natur aus zuverlässiger, da sie nicht von der Verfügbarkeit eines externen Systems abhängen. Außerdem besteht ein geringeres Risiko, dass die Suchtabelle geändert wird, wenn sie sich in einer flachen Datei befindet, die Sie lokal steuern.
...
Unsere Beispielumwandlung durchsucht die Protokolleinträge nach dem Feld s-dns und, falls vorhanden, die Suchtabelle VISUAL.LOOKUP wird verwendet, um den Eintrag s-dns mit den Einträgen im [!DNL OLDDNS] -Spalte der Tabelle. Wenn sich eine Zeile in der Tabelle befindet, erhält das Ausgabefeld s-dns den aktualisierten DNS-Datensatz-Eintrag aus der [!DNL NEWDNS] Spalte der identifizierten Zeile.
